...
class GumTreeUK:
    ...
    def parse_listing(self, resp_text):
        ...
        for ad in ads:
            ...
            image = (
                ad.find(image_xpath, ns).get("href")
                if ad.find(image_xpath, ns)
                else None
            )
            ...
            mileage = (
                ad.find("ns3:attribute/ns3:value", ns).text
                # if ad.find('ns3:attribute[@name="vehicle_mileage"]', ns)
                # else None
            )
            print(mileage)
            year = (
                ad.findall('ns3:attribute[@name="vehicle_registration_year"]', ns)
                # if ad.find('ns3:attribute[@name="vehicle_registration_year"]', ns)
                # else None
            )
